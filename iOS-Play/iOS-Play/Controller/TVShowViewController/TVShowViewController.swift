...
final class TVShowViewController: UIViewController {
    ...
    private let segmentItems = FeedType.tvShows.item
    
    private var itemPicked: [Bool] = {
        var item = [Bool](repeating: false, count: FeedType.tvShows.item.count)
        item[0] = true
        return item
    }()
    ...
    func configSegment() {
        segmentCollection.do {
            ...
            let layout = UICollectionViewFlowLayout().then {
                ...
                $0.itemSize = CGSize(width: view.frame.width / 2, height: 40)
    ...
    func configChildView() {
        let episodes = TopTVEpisodesViewController()
        let seasons = TopTVSeasonsViewController()
...
extension TVShowViewController: UIPageViewControllerDelegate, UIPageViewControllerDataSource {
